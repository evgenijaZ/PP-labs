...
HANDLE Event_input[3], Sem_merge[2], Sem_calc[4], Sem_copy, Mutex_copy;
...
void T1() {
	...
	//copy d
	WaitForSingleObject(Sem_copy, INFINITE);
	d1 = d;
	ReleaseSemaphore(Sem_copy, 1, NULL);

	//copy x
	WaitForSingleObject(Mutex_copy, INFINITE);
	x1 = x;
	ReleaseMutex(Mutex_copy);
	...
	//Signal to T2,T3,T4
	ReleaseSemaphore(Sem_calc[0], 4, NULL);

	//Wait for calculating in T2,T3,T4
	WaitForMultipleObjects(4, Sem_calc, false, INFINITE);

	//Wait for T2,T3
	WaitForMultipleObjects(2, Sem_merge, true, INFINITE);
...
void T2() {
	...
	//Signal to T1,T2,T3
	ReleaseSemaphore(Sem_calc[1], 4, NULL);
	...
	//Signal to T1
	ReleaseSemaphore(Sem_merge[0], 1, NULL);
...
void T3() {
	...
	ReleaseSemaphore(Sem_calc[2], 4, NULL);
	...
	//Signal to T1
	ReleaseSemaphore(Sem_merge[1], 1, NULL);
...
void T4() {
	...
	//Signal to T1,T2,T3
	ReleaseSemaphore(Sem_calc[3], 4, NULL);

	std::cout << "T4 finished\n";
}
//_______________________________________________________________________
//
//---------------------------------MAIN----------------------------------
...
int main()
{
	...
	Sem_copy = CreateSemaphore(NULL, 1, 1, NULL);
	Mutex_copy = CreateMutex(NULL, false, NULL);

	Sem_calc[0] = CreateSemaphore(NULL, 0, 4, NULL);
	Sem_calc[1] = CreateSemaphore(NULL, 0, 4, NULL);
	Sem_calc[2] = CreateSemaphore(NULL, 0, 4, NULL);
	Sem_calc[3] = CreateSemaphore(NULL, 0, 4, NULL);

	Sem_merge[0] = CreateSemaphore(NULL, 0, 1, NULL);
	Sem_merge[1] = CreateSemaphore(NULL, 0, 1, NULL);
	...
	CloseHandle(Sem_merge[0]);
	CloseHandle(Sem_merge[1]);
